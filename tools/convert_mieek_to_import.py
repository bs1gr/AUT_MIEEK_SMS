...
class MIEEKToSMSConverter:
    ...
    def extract_description(self, course_data: Dict) -> str:
        ...
        if "Περιεχόμενο\nΜαθήματος" in course_data or "Περιεχόμενο Μαθήματος" in course_data:
            ...
            if isinstance(content, list):
                # Filter out metadata
                content = " ".join(str(x) for x in content if str(x).strip() and x not in ["Μεθοδολογία", "Διδασκαλίας", "Βιβλιογραφία", "Βασική:"])
            if content:
                parts.append(f"\n**Περιεχόμενο:** {content}")
    ...
    def convert_and_save(self):
        ...
        print(f"\n{'='*60}")
        print(f"MIEEK to SMS Import Converter")
        print(f"{'='*60}")
        ...
        if converted and output_file is not None:
            ...
            print(f"3. Or use the API: POST /api/v1/imports/courses\n")
