...
Greek Αξιολόγηση entries), prefer the specialized converter:
    tools/convert_mieek_to_import.py

Usage:
    python tools/convert_pdf_to_import.py --input data.json --type courses --output templates/courses/
    python tools/convert_pdf_to_import.py --input students.json --type students --output templates/students/
...
class PDFToSMSConverter:
    ...
    def parse_evaluation_rules(self, rules: Any) -> List[Dict[str, Any]]:
        ...
        if isinstance(rules, list):
            # Skip metadata entries
            metadata_terms = [
                "Γλώσσα", "Ελληνική", "Αγγλική", "Κωδικός", "Μαθήματος",
                "Τίτλος Μαθήματος", "Κωδικός Μαθήματος", "Τύπος Μαθήματος",
                "Υποχρεωτικό", "Επίπεδο", "Έτος/Εξάμηνο", "Φοίτησης",
                "Όνομα Διδάσκοντα", "Επίπεδο 5 του Εθνικού Πλαισίου Προσόντων"
            ]
    ...
    def convert_and_save(self):
        ...
        print(f"\n{'='*60}")
        print(f"PDF to SMS Import Converter")
        print(f"{'='*60}")
        ...
        if converted:
            ...
            print(f"2. Use the SMS Operations panel to import the data")
...
Students:
  {
    ...
    parser.add_argument(
        '--input', '-i',
        required=True,
        help='Input JSON file with PDF-extracted data'
    )
