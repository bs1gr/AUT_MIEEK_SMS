...
async function loginAsTeacher(page: Page) {
  ...
  const password = 'Teacher-Pass-1!'; // pragma: allowlist secret
...
test.describe('Student Management - Critical Flows', () => {
  ...
  test('should create a new student successfully', async ({ page }) => {
    ...
    // Clean up: delete student if it already exists (idempotent test)
    const apiBase = process.env.E2E_API_BASE || 'http://localhost:8000';
    try {
      // Try to find the student by email or student_id (API may vary)
      const resp = await page.request.get(`${apiBase}/api/v1/students/?email=${encodeURIComponent(student.email)}`);
      if (resp.ok()) {
        const data = await resp.json();
        if (Array.isArray(data.items)) {
          for (const s of data.items) {
            if (s.email === student.email || s.student_id === student.studentId) {
              await page.request.delete(`${apiBase}/api/v1/students/${s.id}`);
            }
          }
        }
      }
    } catch (e) {
      // Ignore errors (student may not exist)
    }
    ...
    // Wait for modal heading (EN/EL)
    await page.waitForSelector('text=/Add New Student|Προσθήκη Νέου Φοιτητή/i');

    // Fill form
    await page.fill('input[name="first_name"]', student.firstName);
    await page.fill('input[name="last_name"]', student.lastName);
    await page.fill('input[name="email"]', student.email);
    await page.fill('input[name="student_id"]', student.studentId);
