...
const DevToolsPanel = ({ variant = 'standalone', onToast }: DevToolsPanelProps) => {
  ...
  const handleBackup = async () => {
    ...
    try {
      ...
      onToast({ message: t('utils.backupSuccess'), type: 'success' });
  ...
  const handleRestore = async () => {
    ...
    try {
      ...
      onToast({ message: t('utils.restoreSuccess'), type: 'success' });
      if (data && typeof (data as any)?.uptime !== 'undefined') {
        updateUptime((data as any).uptime, (data as any)?.timestamp);
      }
    } catch (error) {
  ...
  return (
    <section className="space-y-6">
      <div className="overflow-hidden rounded-2xl border border-slate-200 shadow-sm">
        ...
        <div className="grid grid-cols-1 gap-4 bg-white p-4 sm:p-6 md:grid-cols-3">
          <div className={subtleCardClass}>
            <div className="text-xs text-slate-500">{t('controlPanel.database')}</div>
            <div className="text-sm font-semibold text-slate-800">{databaseName}</div>
          </div>
          <div className={subtleCardClass}>
            <div className="text-xs text-slate-500">{t('students')}</div>
            <div className="text-sm font-semibold text-slate-800">{studentCount ?? t('na')}</div>
          </div>
          <div className={subtleCardClass}>
            <div className="text-xs text-slate-500">{t('courses')}</div>
            <div className="text-sm font-semibold text-slate-800">{courseCount ?? t('na')}</div>
          </div>
          {typeof uptimeDisplayValue === 'number' && (
            <div className={`${subtleCardClass} md:col-span-3`}>
              <div className="text-xs text-slate-500">{t('controlPanel.uptime')}</div>
              <div className="text-sm font-semibold text-slate-800">{uptimeDisplayValue}s</div>
            </div>
          )}
          <div className={`${subtleCardClass} md:col-span-3`}>
            <div className="text-sm font-semibold text-slate-800">{t('controlPanel.availableEndpoints')}</div>
            <div className="text-xs text-slate-500">{t('utils.quickLinks')}</div>
            {ipList.length === 0 ? (
              <div className="mt-2 text-xs text-slate-500">{t('controlPanel.noIpsAvailable')}</div>
            ) : (
              <div className="mt-3 grid grid-cols-1 gap-3 md:grid-cols-2">
                {ipList.map((ip) => (
                  <div key={ip} className="space-y-1">
                    <div className="font-mono text-xs text-slate-600">{ip}</div>
          ...
          <div className={`${subtleCardClass} md:col-span-3`}>
            <div className="text-xs uppercase tracking-wide text-slate-500">{t('utils.systemStatus')}</div>
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
        <div className={cardClass}>
          <h4 className="mb-2 text-sm font-semibold text-slate-900">{t('utils.backupDatabase')}</h4>
          <p className="mb-4 text-xs text-slate-600">{t('utils.backupDesc')}</p>
          <button
            type="button"
        ...
        <div className={cardClass}>
          <h4 className="mb-2 text-sm font-semibold text-slate-900">{t('utils.resetDatabase')}</h4>
          <p className="mb-3 text-xs text-slate-600">{t('utils.uploadPreviouslySavedBackup')}</p>
          <div className="flex flex-col items-stretch gap-2 md:flex-row md:items-center">
            <label className="flex flex-1 cursor-pointer items-center gap-2 rounded border border-slate-300 bg-slate-100 px-3 py-2 text-xs font-medium text-slate-700 transition hover:bg-slate-200">
          ...
          <p className="mt-2 text-xs text-amber-600">{t('utils.appMayNeedRefresh')}</p>
        </div>

        <div className={cardClass}>
          <h4 className="mb-2 text-sm font-semibold text-slate-900">{t('utils.clear')}</h4>
          <p className="mb-3 text-xs text-slate-600">{t('utils.deleteDataChooseScope')}</p>
          ...
          <select
            ...
            className="mb-3 w-full rounded border border-slate-300 px-2 py-1 text-sm text-slate-700"
        ...
        <div className={cardClass}>
          <h4 className="mb-2 text-sm font-semibold text-slate-900">{t('utils.uploadJsonToImport')}</h4>
          ...
          <p className="mt-2 text-xs text-slate-600">{t('utils.selectJsonFiles')}</p>
        </div>
      </div>

      {result && (
        <div className="rounded-2xl border border-slate-900 bg-slate-950/95 p-4 text-slate-100 shadow-inner">
