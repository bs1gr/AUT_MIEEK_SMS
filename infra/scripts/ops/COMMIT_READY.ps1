...
.PARAMETER Mode
    Execution mode:
    - 'quick'    : Lint + the full backend batch suite with fast-fail, plus frontend
                   tests - ~8-9 min on this repo, not the 2-3 min once claimed here.
                   It is only short when the change touches test files it can scope to.
    ...
    NOTE: The validation checkpoint has no time limit, but it is bound to the content it
    validated. The pre-commit hook compares the tree being committed against the
    checkpoint and blocks if anything changed since - so a checkpoint from last week is
    fine if nothing moved, and one from a minute ago is refused if a file was edited
    after the run. See scripts/ENFORCE_COMMIT_READY_GUARD.ps1.
...
function Get-ChangedFiles {
    ...
    # Staged AND unstaged, not one or the other: this used to look at unstaged files only
    # when nothing was staged, so a partially staged working tree reported a subset and
    # the quick scope could run tests for the wrong files.
    $files = @()
    try {
    ...
    try {
        $files = @($files + @(git diff --name-only 2>$null))
    } catch {}
...
# 1) Ensure the COMMIT_READY checkpoint covers the content being committed
...
# 3) Chain a genuine legacy hook, if the developer had one before this was installed.
#    Skip it when it is one of our own generated guards: an earlier install saved this
#    very hook as pre-commit-legacy, so every commit ran the checkpoint and the version
#    validator twice, in four separate pwsh processes.
LEGACY_HOOK="$HOOKS_DIR/pre-commit-legacy"
if [ -f "$LEGACY_HOOK" ] && ! grep -q "Auto-generated by COMMIT_READY.ps1" "$LEGACY_HOOK"; then
    bash "$LEGACY_HOOK" "$@"
    exit $?
fi
...
function Invoke-CodeQualityChecks {
    ...
    try {
        ...
        if ($npxAvailable) {
            ...
            # markdownlint --fix rewrites every **/*.md in the repo, including files that
            # have nothing to do with the commit being prepared. It is still worth doing,
            # but a validation gate must not edit the tree silently: snapshot the markdown
            # state so the run can say exactly what it touched.
            function Get-MarkdownState {
                $state = @{}
                try {
                    # vs HEAD, so staged edits count too: plain `git diff` would go blank
                    # the moment the developer ran `git add`.
                    foreach ($line in @(git diff HEAD --numstat -- '*.md' 2>$null)) {
                        $parts = $line -split "`t"
                        if ($parts.Count -ge 3) { $state[$parts[2]] = "$($parts[0])/$($parts[1])" }
                    }
                } catch {}
                try {
                    foreach ($path in @(git ls-files --others --exclude-standard -- '*.md' 2>$null)) {
                        if ($path -and (Test-Path $path)) {
                            $state[$path] = (Get-FileHash -LiteralPath $path -Algorithm SHA256).Hash
                        }
                    }
                } catch {}
                return $state
            }
            $mdStateBefore = Get-MarkdownState
            ...
            # Report what the fix pass rewrote, so nothing lands in a commit unannounced.
            $mdStateAfter = Get-MarkdownState
            $mdTouched = @(
                $mdStateAfter.Keys | Where-Object { -not $mdStateBefore.ContainsKey($_) -or $mdStateBefore[$_] -ne $mdStateAfter[$_] }
            ) + @(
                $mdStateBefore.Keys | Where-Object { -not $mdStateAfter.ContainsKey($_) }
            ) | Sort-Object -Unique
            if ($mdTouched.Count -gt 0) {
                Write-Warning-Msg "markdownlint --fix rewrote $($mdTouched.Count) file(s) - review before committing:"
                $mdTouched | Select-Object -First 15 | ForEach-Object { Write-Host "     $_" -ForegroundColor Yellow }
                if ($mdTouched.Count -gt 15) { Write-Host "     ... and $($mdTouched.Count - 15) more" -ForegroundColor Yellow }
            }
        } else {
        ...
        if ($checkExitCode -eq 0) {
            ...
            # Docs lint used to be non-blocking in 'full' mode unless STRICT_DOCS_LINT=1,
            # which made the most thorough mode the most permissive - the opposite of what
            # anyone running -Full expects. It now blocks in every mode, with
            # ALLOW_DOCS_LINT_FAILURE=1 as the explicit, deliberate opt-out.
            $allowDocsFailure = ($env:ALLOW_DOCS_LINT_FAILURE -as [string]) -and ($env:ALLOW_DOCS_LINT_FAILURE.ToLower() -in @('1','true','yes'))
            if ($allowDocsFailure) {
                Write-Warning-Msg "Markdown linting reported issues (non-blocking: ALLOW_DOCS_LINT_FAILURE is set). Review output below:"
                Write-Host $checkOutput -ForegroundColor Gray
                Add-Result "Linting" "Markdown Lint (non-blocking)" $true "Issues present; ALLOW_DOCS_LINT_FAILURE is set"
            } else {
...
function Invoke-TestSuite {
    ...
    try {
        ...
        if ($scopeQuick -and $changedFiles.Count -gt 0) {
            # git reports paths from the repo root, so these must be src/frontend/src/...
            # The pre-flatten 'frontend/src/...' prefix left here since June 2026 matched
            # nothing, which is why quick mode always ran the whole frontend suite.
            $changedFrontendTests = $changedFiles | Where-Object { $_ -match '^src/frontend/src/.+\.(test|spec)\.(ts|tsx|js|jsx)$' -or $_ -match '^src/frontend/src/.+/__tests__/.+\.(ts|tsx|js|jsx)$' }
        }
...
function New-CommitMessage {
    ...
    if ($script:Results.Overall) {
        ...
        Write-Host ""
        Write-Host "Note: The checkpoint has no time limit, but it covers the files as they are now." -ForegroundColor Cyan
        Write-Host "      Edit one of them before committing and the hook will ask you to re-run." -ForegroundColor Cyan
    } else {
    ...
    if ($Mode -eq 'cleanup') {
        ...
        if (-not $SkipLint) {
            if ($AutoFix) {
                ...
                Invoke-PreCommitHookValidation | Out-Null
            } else {
                Invoke-PreCommitHookValidation | Out-Null
                Invoke-CodeQualityChecks | Out-Null
            }
        ...
        # Out-Null on every phase call: an unsuppressed one puts its return value in this
        # function's output, and the entry point then receives an array instead of the
        # exit code it returns below.
        Invoke-DocumentationCheck | Out-Null
    }
...
# ============================================================================
try {
    # Fingerprint before anything runs, so the end-of-run checkpoint can say whether the
    # tree it is blessing is the tree that was actually linted and tested.
    $guardScript = Join-Path $PROJECT_ROOT "scripts\ENFORCE_COMMIT_READY_GUARD.ps1"
    $startFingerprint = $null
    if (Test-Path $guardScript) {
        try { $startFingerprint = (& $guardScript -PrintFingerprint) } catch {}
    }

    # Invoke-MainWorkflow's callees leak values into the success stream, so this returns
    # something like @($true, 0) rather than a bare 0. `@($true,0) -eq 0` yields @(0),
    # which PowerShell then evaluates as FALSE - so the checkpoint block below never ran,
    # and the whole gate rested on a stale .commit-ready-validated file (an empty one, on
    # this machine) that the old guard accepted because it only checked for existence.
    # Take the last emitted value, which is the `return` the function intended.
    $workflowResult = Invoke-MainWorkflow
    $exitCode = if ($workflowResult -is [System.Array]) { $workflowResult[-1] } else { $workflowResult }
    try { $exitCode = [int]$exitCode } catch { $exitCode = 1 }
    ...
    if ($exitCode -eq 0) {
        # A checkpoint may only be written by a run that actually validated something.
        # -Mode cleanup returns 0 from cleanup results alone, and -SkipTests/-SkipLint
        # short-circuit the checks, so those runs must not mint one.
        if ($Mode -eq 'cleanup') {
            Write-Host ""
            Write-Host "ℹ️  Cleanup-only run - no checkpoint written (nothing was validated)" -ForegroundColor Cyan
        }
        elseif ($SkipTests -or $SkipLint) {
            Write-Host ""
            Write-Host "⚠️  No checkpoint written: -SkipTests/-SkipLint means this run did not validate the tree." -ForegroundColor Yellow
            Write-Host "   The pre-commit hook will block until a full run passes." -ForegroundColor Yellow
        }
        else {
            Write-Host ""
            Write-Host "🔒 Recording COMMIT_READY validation checkpoint..." -ForegroundColor Cyan

            # The checkpoint covers the tree as it is now, at the end of the run. If that
            # is not the tree the checks actually saw - because a file was edited while
            # this ran, or because an auto-fix above rewrote something - say so rather
            # than blessing it silently.
            if ($startFingerprint) {
                $endFingerprint = $null
                try { $endFingerprint = (& $guardScript -PrintFingerprint) } catch {}
                if ($endFingerprint -and ($endFingerprint -ne $startFingerprint)) {
                    Write-Host "   ⚠️  Files changed while this run was in progress (or were auto-fixed above)." -ForegroundColor Yellow
                    Write-Host "      The checkpoint covers the CURRENT contents, which the checks above did not all see." -ForegroundColor Yellow
                }
            }

            # Absolute path: invoked as .\scripts\... this silently did nothing whenever
            # COMMIT_READY was run from anywhere but the project root.
            & (Join-Path $PROJECT_ROOT "scripts\ENFORCE_COMMIT_READY_GUARD.ps1") -Mode $Mode
            Write-Host "   ✅ Checkpoint recorded - valid until the validated files change" -ForegroundColor Green
        ...
        # Optional: Record a state snapshot to artifacts/state. Outside the branches
        # above: -Snapshot was asked for, so it happens on any successful run.
        if ($Snapshot) {
