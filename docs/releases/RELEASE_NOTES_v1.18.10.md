...
Version 1.18.10 packages the post-vvvvv1.18.25 installer patch work into a clean release lineage. The focus is preserving existing PostgreSQL profile settings during upgrades, avoiding silent fallback to local SQLite, and shipping a refreshed installer artifact that matches the corrected upgrade behavior.
...
## 🧹 Repository Maintenance Since vvvvv1.18.25
...
- Release/tag lineage reviewed: `vvvvv1.18.25..HEAD`
- Post-release commits reviewed: 5
- Confirmed that prior installer rebuild commit was **not** inside `vvvvv1.18.25` tag lineage
- Version metadata updated for `vvvvv1.18.25` release preparation

## Commits in Scope

- `500147165` — `build(installer): rebuild vvvvv1.18.25 installer with profile-drift fix and updated docs`
...
- `d555722b4` — `style(docs): fix release notes formatting and CSV newlines for vvvvv1.18.25`
