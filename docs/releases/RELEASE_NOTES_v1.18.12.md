...
**Previous Archived Prerelease Reference**: vvvvv1.18.25
**Release Type**: Corrective Patch Release → First Official Public Release

## Summary

Version 1.18.12 is the clean corrective release after the exact-tag cleanup that archived `vvvvv1.18.25` and `vvvvv1.18.25` as prereleases. It restores a reliable installer publication path, hardens security-sensitive maintenance and backup path handling, and aligns update metadata with installer-only GitHub releases that rely on GitHub digest metadata instead of `.sha256` sidecars. On March 11, 2026, this exact verified tag was promoted to become the **first official public release**.
...
- Reused the verified `vvvvv1.18.25` tag as the official public release without minting a new version.
...
- Fresh local installer verification for `vvvvv1.18.25` completed successfully:
...
- `fix(release): prepare verified vvvvv1.18.25 corrective candidate`
- `docs(release): record installer guardrails`
- `build(release): add installer tracked-input guardrails`
- `docs(release): record vvvvv1.18.25 publication status`
- `docs(release): finalize vvvvv1.18.25 official publication path`
- `docs(release): sync official vvvvv1.18.25 publication state`
