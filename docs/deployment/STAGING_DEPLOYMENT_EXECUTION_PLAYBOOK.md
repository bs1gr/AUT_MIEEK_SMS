...
**Reference**: [STAGING_DEPLOYMENT_PLAN_$11.15.1.md](STAGING_DEPLOYMENT_PLAN_$11.15.1.md)
...
10:00 - 10:45: Deploy $11.15.1 to staging (45 min)
...
Test-Path "docs/releases/RELEASE_NOTES_$11.15.1.md"  # Should exist
Test-Path "docs/deployment/STAGING_DEPLOYMENT_PLAN_$11.15.1.md"  # Should exist
...
Copy-Item "data/student_management.db" "backend/backups/pre_$11.15.1_backup_$timestamp.db.bak"
Write-Host "✅ Backup created: pre_$11.15.1_backup_$timestamp.db.bak"

# Verify backup
$backupSize = (Get-Item "backend/backups/pre_$11.15.1_backup_$timestamp.db.bak").Length / 1MB
...
### Step 1.4: Deploy $11.15.1 (5 minutes)
```powershell
# Start $11.15.1 (will build if needed)
...
**Deployment Phase Complete**: ✅ $11.15.1 running and healthy
...
Copy-Item "backend/backups/pre_$11.15.1_backup_20260108_100000.db.bak" "data/student_management.db" -Force
Write-Host "✅ Database restored from backup"
```

### Step 3: Redeploy $11.15.1 (5 min)
```powershell
...
Write-Host "Rollback complete - $11.15.1 restored"
```
