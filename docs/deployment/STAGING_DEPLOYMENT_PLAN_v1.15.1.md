...
**Rollback Time**: <5 minutes (revert to v1.15.0 if needed)
...
```bash
# Backup current database (v1.15.0)
mkdir -p /staging/backups/pre-1.15.1
docker exec sms-fullstack sqlite3 /data/student_management.db ".backup '/staging/backups/pre-1.15.1/student_management_v1.15.0.db'"
...
If critical issues found, rollback to v1.15.0:
...
docker exec sms-fullstack sqlite3 /data/student_management.db ".restore '/staging/backups/pre-1.15.1/student_management_v1.15.0.db'"

# Checkout previous version
git checkout v1.15.0  # Or git checkout <commit-hash>
