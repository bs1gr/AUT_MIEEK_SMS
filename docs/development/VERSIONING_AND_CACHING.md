...
> **Note (v1.18.3+)**: The scripts `RUN.ps1` and `SMART_SETUP.ps1` referenced in this document have been consolidated into `DOCKER.ps1`. Use `DOCKER.ps1 -Start`, `DOCKER.ps1 -Update`, and `DOCKER.ps1 -UpdateClean` instead. The version verification logic described here is now built into `DOCKER.ps1`.
...
2. **Tag releases in Git**
   ...
   ```powershell
   git tag -a v1.18.3 -m "Release version 1.7.0"
