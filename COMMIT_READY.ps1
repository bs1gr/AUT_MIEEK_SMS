...
function Invoke-PreCommitHookValidation {
    ...
    if ($precommitAvailable) {
        ...
        if (Test-Path (Join-Path $SCRIPT_DIR ".git")) {
            $hookContent = "#!/bin/sh`n# Auto-generated by COMMIT_READY.ps1`nexec pwsh -NoProfile -ExecutionPolicy Bypass -File `"./COMMIT_READY.ps1`" -Mode quick"
            ...
            if ($currentHookNorm -ne $hookContentNorm) {
                Write-Info "Updating .git/hooks/pre-commit to use COMMIT_READY.ps1 wrapper..."
...
function Invoke-VersionFormatValidation {
    ...
    foreach ($pattern in $forbiddenPatterns.Keys) {
        if ($version -match $pattern) {
            ...
            Write-Host '  Required: v1.x.x format (e.g., v1.17.2)' -ForegroundColor Red
            Write-Host ""
    ...
    if ($version -match '^v?1\.\d+\.\d+$') {
        ...
        Write-Host '  Required: v1.x.x or 1.x.x format (e.g., v1.17.2 or 1.18.0)' -ForegroundColor Red
