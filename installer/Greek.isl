...
; SMS custom installer messages
DockerRequired=Απαιτείται Docker Desktop
DockerNotFound=Το Docker Desktop δεν εντοπίστηκε.%n%nΘέλετε να ανοίξει η σελίδα λήψης του Docker Desktop;
LaunchApp=Εκκίνηση SMS μετά την εγκατάσταση
CreateShortcut=Δημιουργία συντόμευσης στην επιφάνεια εργασίας
BuildContainer=Δημιουργία Docker container (πρώτη εκτέλεση, ~5-10 λεπτά)
Prerequisites=Προαπαιτούμενα:
OpenDockerPage=Άνοιγμα σελίδας λήψης Docker Desktop
DockerInstalled=Το Docker Desktop είναι εγκατεστημένο και έτοιμο
DockerNotInstalled=Το Docker Desktop ΔΕΝ είναι εγκατεστημένο
DockerRunning=Το Docker Desktop εκτελείται
DockerNotRunning=Το Docker Desktop δεν εκτελείται - παρακαλώ ξεκινήστε το
BuildingContainer=Δημιουργία SMS Docker container...
FirstRunNote=Η πρώτη εκκίνηση θα δημιουργήσει το container (διαρκεί 5-10 λεπτά)
ExistingInstallDetected=Εντοπίστηκε Υπάρχουσα Εγκατάσταση
ExistingVersionFound=Η έκδοση %1 είναι ήδη εγκατεστημένη στο:%n%2%n%nΤι θέλετε να κάνετε;
SameVersionFound=Η έκδοση %1 είναι ήδη εγκατεστημένη στο:%n%2%n%nΤι θέλετε να κάνετε;
UpgradeOption=Ενημέρωση/Αντικατάσταση (διατήρηση δεδομένων και ρυθμίσεων)
CleanInstallOption=Νέα εγκατάσταση (αφαίρεση της προηγούμενης εγκατάστασης)
CancelOption=Ακύρωση εγκατάστασης
UpgradingFrom=Ενημέρωση από έκδοση %1 σε %2
BackupCreated=Δημιουργήθηκε αντίγραφο ασφαλείας στο: %1
KeepUserData=Διατήρηση δεδομένων χρήστη (βάση, αντίγραφα ασφαλείας, ρυθμίσεις)
RemoveOldVersion=Αφαίρεση προηγούμενης έκδοσης πριν την εγκατάσταση
UpgradePrompt=Πατήστε ΝΑΙ για ενημέρωση/αντικατάσταση, ΟΧΙ για νέα εγκατάσταση ή ΑΚΥΡΟ για έξοδο.
KeepDataPrompt=Θέλετε να διατηρήσετε τα δεδομένα σας (βάση, αντίγραφα ασφαλείας, ρυθμίσεις);%n%nΠατήστε ΝΑΙ για διατήρηση δεδομένων για μελλοντική επανεγκατάσταση.%nΠατήστε ΟΧΙ για πλήρη διαγραφή.
ViewReadme=Προβολή τεκμηρίωσης README
DockerStatusTitle=Έλεγχος Docker Desktop
...
InstallTypeDesc=Επιλέξτε ποια έκδοση του SMS θέλετε να εγκαταστήσετε
InstallTypeIntro=Επιλέξτε την έκδοση του Συστήματος Διαχείρισης Μαθητών (SMS) που θέλετε να εγκαταστήσετε:
InstallDockerEdition=Έκδοση Docker Production (Συνιστάται)
InstallDockerEditionDesc=Πλήρης ανάπτυξη παραγωγής με Docker containers. Περιλαμβάνει όλες τις δυνατότητες, αυτόματες ενημερώσεις και περιβάλλον ενσωματωμένο σε δοχείο. Απαιτεί Docker Desktop.
InstallDockerOnly=Μόνο Docker Production (Συνιστάται)
InstallDockerOnlyDesc=Ελάχιστη εγκατάσταση με Docker container (ταχύτερη, καθαρότερη)
InstallDevEnvironment=Συμπερίληψη Περιβάλλοντος Ανάπτυξης
InstallDevEnvironmentDesc=Προσθήκη Node.js, Python και αρχείων για τοπική ανάπτυξη
InstallLiteEdition=Έκδοση Native Lite (Ελαφρή Αυτόνομη)
InstallLiteEditionDesc=Ελαφρή αυτόνομη εφαρμογή (68 MB). Εκτελείται σε Windows χωρίς Docker. Υποστηρίζει τοπική SQLite και προαιρετική ενσωμάτωση QNAP PostgreSQL. Ιδανική για ατομικούς υπολογιστές.
DbConfigPageTitle=Ρύθμιση Βάσης Δεδομένων
DbConfigPageSubtitle=Σύνδεση με την κοινόχρηστη βάση QNAP PostgreSQL (συνιστάται) ή χρήση τοπικής SQLite ως εναλλακτική.
DbConfigIntro=Επιλέξτε πώς να συνδεθείτε στα δεδομένα των μαθητών. Η κοινόχρηστη βάση QNAP συνιστάται για τις περισσότερες εγκαταστάσεις.
DbLocalOption=Τοπική SQLite (εναλλακτική όταν το QNAP δεν είναι διαθέσιμο)
DbLocalHint=Χρησιμοποιήστε μόνο όταν ο διακομιστής QNAP PostgreSQL δεν είναι διαθέσιμος. Τα δεδομένα θα αποθηκευτούν τοπικά και μπορούν να μεταφερθούν αργότερα.
DbRemoteOption=QNAP PostgreSQL (κοινόχρηστη βάση στο NAS - συνιστάται)
DbRemoteHint=Σύνδεση με την κοινόχρηστη βάση PostgreSQL στο QNAP NAS σας. Αυτή είναι η κύρια βάση για όλες τις εγκαταστάσεις.
DbCredentialsSection=Αρχείο διαπιστευτηρίων QNAP PostgreSQL
DbCredentialsFile=Αρχείο διαπιστευτηρίων (.json, .env ή .txt)
DbBrowse=Αναζήτηση...
DbCredentialsHint=Χρησιμοποιήστε το ίδιο αρχείο διαπιστευτηρίων που δέχεται το πάνελ Διαχείρισης Βάσης Δεδομένων.
DbNoFileSelected=Δεν έχει επιλεγεί αρχείο διαπιστευτηρίων.
DbLoadedCredentials=Φορτώθηκαν διαπιστευτήρια: %1:%2 / %3 (%4)
DbLoadedExistingCredentials=Χρήση ήδη αποθηκευμένων απομακρυσμένων διαπιστευτηρίων: %1:%2 / %3 (%4)
DbLocalSummary=Προφίλ βάσης: Τοπική SQLite (ενσωματωμένη βάση αρχείου)
DbRemoteSummary=Προφίλ βάσης: QNAP PostgreSQL (%1:%2/%3)
DbFileRequired=Παρακαλώ επιλέξτε αρχείο διαπιστευτηρίων βάσης (.json, .env ή .txt).
