...
.DESCRIPTION
    ...
    - Installer Greek info pages: RTF with ansicpg1253 declarations
    - Greek license: Windows-1253 (CP1253) or UTF-8 with BOM

    Inno Setup 6 requires proper encoding declarations to render Greek text correctly.
    Build pipeline regenerates Greek RTF info files before compilation.
...
.NOTES
    ...
    Created: 2025-12-04
        ...
        - Greek RTF files: ASCII RTF wrapper with CP1253 escaped Greek text
...
$GreekFiles = @(
    ...
    @{
        Path = Join-Path $ScriptDir "installer_welcome_el.rtf"
        Type = "Welcome RTF"
        Encoding = "RTF + CP1253 escapes"
        Description = "Greek welcome message (RTF ansicpg1253)"
    },
    @{
        Path = Join-Path $ScriptDir "installer_complete_el.rtf"
        Type = "Completion RTF"
        Encoding = "RTF + CP1253 escapes"
        Description = "Greek completion message (RTF ansicpg1253)"
    },
    @{
...
function Validate-GreekInfoFile {
    ...
    try {
        if ([System.IO.Path]::GetExtension($Path).ToLowerInvariant() -eq '.rtf') {
            $content = Get-Content $Path -Raw -Encoding ASCII
            $issues = @()

            if ($content -notmatch '^\{\\rtf1') {
                $issues += 'Missing RTF header'
            }
            if ($content -notmatch '\\ansicpg1253') {
                $issues += 'Missing ansicpg1253 declaration'
            }
            if ($content -notmatch "\\'[0-9a-fA-F]{2}") {
                $issues += 'Missing CP1253 escaped characters'
            }

            if ($issues.Count -eq 0) {
                Write-Status OK "Greek RTF structure validated ✓"
                return $true
            }

            Write-Status Warning ($issues -join '; ')
            return $false
        }
...
function Repair-GreekInfoFile {
    ...
    try {
        if ([System.IO.Path]::GetExtension($Path).ToLowerInvariant() -eq '.rtf') {
            Write-Status Warning "$Description is generated by fix_greek_encoding_permanent.py - regenerate instead of in-place fixing"
            return $false
        }
...
for ($i = 1; $i -lt $GreekFiles.Count; $i++) {
    $file = $GreekFiles[$i]
    if (-not (Validate-GreekInfoFile $file.Path $file.Description)) {
        $allPassed = $false
    }
}
Write-Host ""
...
if ($Fix -and -not $allPassed) {
    ...
    if ((Validate-GreekISL $GreekFiles[0].Path) -eq $false) {
        if (Repair-GreekISL $GreekFiles[0].Path) {
            $fixedCount++
        }
    }
    Write-Host ""
    ...
    for ($i = 1; $i -lt $GreekFiles.Count; $i++) {
        $file = $GreekFiles[$i]
        if ((Validate-GreekInfoFile $file.Path $file.Description) -eq $false) {
            if (Repair-GreekInfoFile $file.Path $file.Description) {
                $fixedCount++
            }
        }
    }
