...
PR body (short)
---------------
This PR adds CI-level mypy checks, a secret-guard to prevent insecure `SECRET_KEY` usage in CI, redirects test backups to temporary directories, runs safe ruff fixes, and reduces noisy health-check logging. The changes are conservative and were validated by running the backend tests and `mypy` locally.
