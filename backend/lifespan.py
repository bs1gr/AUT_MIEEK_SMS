...
STARTUP_DEBUG = os.environ.get("STARTUP_DEBUG", "0").strip().lower() in {
    "1",
    "true",
    "yes",
    "debug",
...
def get_lifespan():
    ...
    async def lifespan(app):
        ...
        disable_startup = os.environ.get(
        ...
        if not disable_startup:
            ...
            threading.Thread(
                target=_migrate_bg, daemon=True, name="migrations-bg"
            ).start()
            if getattr(settings, "DEFAULT_ADMIN_EMAIL", None) and getattr(
