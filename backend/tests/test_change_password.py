def test_change_password_flow(client):
    # Register a user
    payload = {"email": "changeme@example.com", "password": "OldPass123!"}
    ...
    r_old = client.post("/api/v1/auth/login", json={"email": payload["email"], "password": "OldPass123!"})
    assert r_old.status_code == 400

    # New password should work
    r_new = client.post("/api/v1/auth/login", json={"email": payload["email"], "password": "NewPass456!"})
