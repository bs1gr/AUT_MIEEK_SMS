...
def test_attendance_invalid_date_range(client):
    ...
    response = client.get(
        "/api/v1/attendance?start_date=2025-12-31&end_date=2025-01-01"
    )
    assert (
...
def test_concurrent_student_creation(client):
    ...
    assert (
        codes.count(200) <= 1
    )  # Only one should succeed, others should fail (duplicate)
...
def test_grade_create_invalid_student(client):
    ...
    )  # API returns 422 Unprocessable Entity for invalid student
...
def test_create_course_with_max_length_fields(client):
    ...
    assert response.status_code in (
        ...
        200,
        422,
    )  # Acceptable: created or validation error
...
def test_access_admin_endpoint_without_auth(client):
    ...
    # Use the shared client fixture, but simulate no auth
    response = client.get("/api/v1/admin/users")
    assert response.status_code in (401, 403)


def test_access_admin_endpoint_with_invalid_token(client):
...
def test_bulk_import_large_number_of_students(client):
    ...
    assert response.status_code in (
        ...
        400,
        422,
    )  # Acceptable: created, validation, or error
