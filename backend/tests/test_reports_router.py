...
def test_generate_student_performance_report(client: TestClient, clean_db):
    ...
    # Create test course
    course = Course(
        course_name="Test Course", course_code="CS101", semester="Fall 2024"
    )
    clean_db.add(course)
    clean_db.flush()
...
def test_generate_report_specific_courses(client: TestClient, clean_db):
    ...
    assert (
        "MATH101" in course_codes or len(course_codes) == 0
...
def test_generate_report_without_optional_data(client: TestClient, clean_db):
    ...
    assert data.get("overall_attendance") is None or "attendance_rate" not in data.get(
        "overall_attendance", {}
    )
    assert data.get("overall_grades") is None or "average_grade" not in data.get(
        "overall_grades", {}
    )
