...
def test_enroll_single_student_success(client):
    ...
    # Enroll student
    response = client.post(f"/api/v1/enrollments/course/{course_id}", json={
...
def test_enroll_duplicate_prevention(client):
    ...
    # First enrollment
    response1 = client.post(f"/api/v1/enrollments/course/{course_id}", json={
        "student_ids": [student_id]
    })
    assert response1.json()["created"] == 1
    
    # Try to enroll again (should skip duplicate)
    response2 = client.post(f"/api/v1/enrollments/course/{course_id}", json={
        "student_ids": [student_id]
    })
...
def test_enroll_nonexistent_course(client):
    ...
    response = client.post("/api/v1/enrollments/course/99999", json={
        "student_ids": [1]
    })
...
def test_get_all_enrollments(client):
    ...
    # Enroll
    client.post(f"/api/v1/enrollments/course/{course_id}", json={
        "student_ids": [student_id]
    })
