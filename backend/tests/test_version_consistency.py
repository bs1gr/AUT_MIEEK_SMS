...
def test_version_file_exists(project_root: Path):
    ...
    assert re.match(
        r"^\d+\.\d+\.\d+$", content
    ), f"VERSION file must contain valid semver (got: {content})"
...
def test_frontend_package_json_version(project_root: Path, version_file: str):
    ...
    ), f"package.json version ({pkg_version}) != VERSION ({version_file})"
...
def test_backend_main_docstring_version(project_root: Path, version_file: str):
    ...
    ), "Version not found in backend/main.py docstring"
    assert (
...
def test_developer_guide_version(project_root: Path, version_file: str):
    ...
    assert (
        extracted_version is not None
    ), "Version not found in DEVELOPER_GUIDE_COMPLETE.md"
    assert (
        extracted_version == version_file
...
def test_documentation_index_version(project_root: Path, version_file: str):
    ...
    assert (
        extracted_version is not None
    ), "Version not found in docs/DOCUMENTATION_INDEX.md"
    assert (
        extracted_version == version_file
...
def test_root_documentation_index_version(project_root: Path, version_file: str):
    ...
    extracted_versions = re.findall(
        r"\*\*Version:\*\*\s*(\d+\.\d+\.\d+)|\(v(\d+\.\d+\.\d+)\)", header_lines
    )
...
def test_installer_wizard_version(project_root: Path, version_file: str):
    ...
    assert (
        extracted_version is not None
    ), "Version not found in SMS_INSTALLER_WIZARD.ps1"
    assert (
        extracted_version == version_file
...
def test_uninstaller_wizard_version(project_root: Path, version_file: str):
    ...
    assert (
        extracted_version is not None
    ), "Version not found in SMS_UNINSTALLER_WIZARD.ps1"
    assert (
        extracted_version == version_file
...
def test_all_versions_consistent(project_root: Path, version_file: str):
    ...
    if main_path.exists():
        content = main_path.read_text(encoding="utf-8")
        version_files["main.py"] = extract_version(
            content, r"Version:\s*(\d+\.\d+\.\d+)"
        )
    ...
    for doc_key, doc_path in [
        ...
        (
            "DEVELOPER_GUIDE",
            project_root / "docs" / "development" / "DEVELOPER_GUIDE_COMPLETE.md",
        ),
        ...
        if doc_path.exists():
            ...
            version_files[doc_key] = extract_version(
                content, r"\*\*?Version\*\*?:\s*(\d+\.\d+\.\d+)"
            )
    ...
    for wizard_key, wizard_path in [
        (
            "INSTALLER_WIZARD",
            project_root / "tools" / "installer" / "SMS_INSTALLER_WIZARD.ps1",
        ),
        (
            "UNINSTALLER_WIZARD",
            project_root / "tools" / "installer" / "SMS_UNINSTALLER_WIZARD.ps1",
        ),
    ]:
        if wizard_path.exists():
            content = wizard_path.read_text(encoding="utf-8")
            version_files[wizard_key] = extract_version(
                content, r'Version\s*=\s*"(\d+\.\d+\.\d+)"'
            )
...
def test_verify_version_script_exists(project_root: Path):
    ...
    assert (
        script_path.exists()
    ), "VERIFY_VERSION.ps1 script must exist for CI/CD version validation"

    # Verify script has proper execution permissions (on Windows, just check readability)
    content = script_path.read_text(encoding="utf-8")
    assert (
        "param(" in content.lower()
    ), "VERIFY_VERSION.ps1 must be a valid PowerShell script"
    assert (
        "version" in content.lower()
    ), "VERIFY_VERSION.ps1 must handle version checking"
