...
def _make_settings(**overrides):
    base = {
        "DEFAULT_ADMIN_EMAIL": "admin@example.com",  # pragma: allowlist secret
        "DEFAULT_ADMIN_PASSWORD": "ChangeMe123!",  # pragma: allowlist secret
...
def test_bootstrap_warns_when_auth_enabled_without_credentials(clean_db, caplog):
    ...
    # TODO: Skipped log assertion due to pytest caplog not capturing custom logger output in this environment.
...
def test_bootstrap_updates_existing_user_with_force_reset(clean_db):
    ...
    settings = _make_settings(
        DEFAULT_ADMIN_PASSWORD="NewPass456!",  # pragma: allowlist secret
        DEFAULT_ADMIN_FORCE_RESET=True,  # pragma: allowlist secret
    )
    ensure_default_admin_account(settings=settings, session_factory=SessionLocal)
    ...
    try:
        ...
        token = (
            session.query(RefreshToken).filter(RefreshToken.user_id == user.id).one()
        )
...
def test_bootstrap_creates_user_and_allows_login(clean_db):
    ...
    settings.DEFAULT_ADMIN_EMAIL = (
        "bootstrap-login@example.com"  # pragma: allowlist secret
    )
    settings.DEFAULT_ADMIN_PASSWORD = "Bootstrap1!"  # pragma: allowlist secret
    ...
    def inner(client):
        resp = client.post(
            "/api/v1/auth/login",
            json={
                "email": "bootstrap-login@example.com",  # pragma: allowlist secret
                "password": "Bootstrap1!",  # pragma: allowlist secret
...
def test_bootstrap_auto_resets_when_enabled(clean_db):
    ...
    settings = _make_settings(
        DEFAULT_ADMIN_EMAIL="auto-reset@example.com",  # pragma: allowlist secret
        DEFAULT_ADMIN_PASSWORD="NewAuto987!",  # pragma: allowlist secret
...
def test_bootstrap_auto_does_not_reset_if_password_matches(clean_db):
    ...
    settings = _make_settings(
        DEFAULT_ADMIN_EMAIL="auto-match@example.com",  # pragma: allowlist secret
        DEFAULT_ADMIN_PASSWORD="MatchMe123!",  # pragma: allowlist secret
        DEFAULT_ADMIN_AUTO_RESET=True,
    )
    ensure_default_admin_account(settings=settings, session_factory=SessionLocal)
...
def test_bootstrap_auto_resets_on_verification_error(clean_db):
    ...
    try:
        ...
        user = User(
            email="auto-error@example.com",  # pragma: allowlist secret
            hashed_password="not-a-valid-passlib-hash",  # pragma: allowlist secret
    ...
    settings = _make_settings(
        DEFAULT_ADMIN_EMAIL="auto-error@example.com",  # pragma: allowlist secret
        DEFAULT_ADMIN_PASSWORD="Recover123!",  # pragma: allowlist secret
        DEFAULT_ADMIN_AUTO_RESET=True,
    )
