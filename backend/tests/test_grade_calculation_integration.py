...
def test_final_grade_with_absence_penalty(client, admin_token, bootstrap_admin):
    ...
    assert grade_resp.status_code in (200, 201), f"Grade creation failed: {grade_resp.status_code} {grade_resp.text}"
    ...
    resp = client.get(f"/api/v1/analytics/student/{student_id}/course/{course_id}/final-grade", headers=headers)
