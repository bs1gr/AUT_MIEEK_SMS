...
def get_all_courses(skip: int = 0, limit: int = 100, semester: Optional[str] = None, db: Session = Depends(get_db)):
    """
    ...
    try:
        ...
        if "evaluation_rules" in update_data and update_data["evaluation_rules"]:
            ...
            if abs(total_weight - 100.0) > 0.01:  # Allow for floating point precision
                ...
                raise HTTPException(
                    status_code=400, detail=f"Evaluation rule weights must sum to 100%. Current sum: {total_weight}%"
                )
...
def update_evaluation_rules(
    ...
    try:
        ...
        return {"course_id": course_id, "evaluation_rules": _normalize_evaluation_rules(db_course.evaluation_rules)}
