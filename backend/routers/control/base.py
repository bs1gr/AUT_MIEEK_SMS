...
async def run_diagnostics():
    ...
    if not in_docker:
        ...
        if node_ok and node_version:
            major = (
                int(node_version.strip("v").split(".")[0])
            ...
            results.append(
                DiagnosticResult(
                    category="Node.js",
                    status="error",
                    message="Node.js not found or not in PATH",
                    details={},
                )
            )
        ...
        if npm_ok:
            ...
            results.append(
                DiagnosticResult(
                    category="npm", status="error", message="npm not found", details={}
                )
            )
...
async def get_environment_info(include_packages: bool = False):
    ...
    if node_ok:
        ...
        success, stdout, _ = run_command(
            ["where" if sys.platform == "win32" else "which", "node"]
        )
        if success:
    ...
    try:
        (create_app,) = __import__(
            "backend.import_resolver"
        ).import_resolver.import_names("main", "create_app")  # type: ignore[attr-defined]
    ...
    try:
        ...
        ok, out, _ = run_command(
            ["git", "describe", "--tags", "--always", "--dirty"], timeout=5
        )
        if ok:
        ...
        if not db_path.exists():
            results.append(
                DiagnosticResult(
                    ...
                    message="Database file not found - will be created on first run",
                    details={
                        "solution": "Start the application to initialize database"
                    },
                )
            )
        else:
            try:
                ...
                results.append(
                    DiagnosticResult(
                        ...
                        details={
                            "solution": "Check database file permissions or restore from backup"
                        },
                    )
                )
    except Exception as e:
    ...
    if not check_docker_running():
        results.append(
            DiagnosticResult(
                ...
                details={
                    "solution": "Start Docker Desktop if you want to run in Docker mode"
                },
            )
        )
