...
async def control_start():
    ...
    try:
        ...
        try:
            ...
            if version_check.returncode != 0:
                raise subprocess.CalledProcessError(
                    version_check.returncode, [npm_cmd, "-v"]
                )
        except Exception:
            return JSONResponse(
                {
        ...
        if not os.path.isdir(node_modules):
            ...
            install_args = (
                [npm_cmd, "ci"] if os.path.isfile(lockfile) else [npm_cmd, "install"]
            )
        ...
        try:
            ...
            return JSONResponse(
                {"success": False, "message": "Failed to start frontend process"},
                status_code=500,
            )
        ...
        try:
            safe_run(
                ["taskkill", "/F", "/T", "/PID", str(FRONTEND_PROCESS.pid)], timeout=3
            )
        except Exception:
            pass
        return JSONResponse(
        ...
        return JSONResponse(
            {"success": False, "message": f"Unexpected error: {e!s}"}, status_code=500
        )
    ...
    try:
        ...
        if port_processes:
            stopped_services.append(
                f"Frontend (processes detected on ports: {list(port_processes.keys())})"
            )
            errors.append(
...
async def control_stop(request: Request, _auth=Depends(require_control_admin)):
    ...
    try:
        ...
        for port in FRONTEND_PORT_CANDIDATES:
            ...
            if pids:
                for pid in pids:
                    try:
                        res = safe_run(
                            ["taskkill", "/F", "/T", "/PID", str(pid)], timeout=5
                        )
                        if getattr(res, "returncode", None) == 0:
                            stopped_any = True
                            stopped_pids.append(pid)
        ...
        message = (
            ...
            if stopped_any
            else "No frontend processes detected"
        )
        ...
        return JSONResponse(
            {
                "success": False,
                "message": f"Frontend stop error: {e!s}",
                "timestamp": datetime.now().isoformat(),
            },
            status_code=500,
        )
...
async def control_stop_backend(request: Request, _auth=Depends(require_control_admin)):
    try:
            ...
            try:
                ...
                if os.name == "nt":
                    for pid in {current_pid, parent_pid}:
                        try:
                            safe_run(
                                ["taskkill", "/F", "/T", "/PID", str(pid)], timeout=3
                            )
                        except Exception:
                            pass
                else:
        ...
        return JSONResponse(
            {
                "success": False,
                "message": f"Backend stop error: {e!s}",
                "timestamp": datetime.now().isoformat(),
            },
            status_code=500,
        )
