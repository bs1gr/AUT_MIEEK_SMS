...
async def install_backend_deps(request: Request):
    ...
    success, stdout, stderr = run_command(
        ["pip", "install", "-r", str(requirements_file)], timeout=300
    )
    if success:
        return OperationResult(
            success=True,
...
async def docker_build(request: Request):
    ...
    success, stdout, stderr = docker_compose(
        ["build", "--no-cache"], cwd=project_root, timeout=600
    )
    if success:
        return OperationResult(
            success=True,
...
class ZipSaveRequest(BaseModel):
    destination: str = Field(
        description="Destination path (directory or full .zip path)"
    )


class ZipSelectedRequest(BaseModel):
    filenames: List[str] = Field(
        description="List of backup .db filenames to include in the ZIP"
    )
...
class BackupCopyRequest(BaseModel):
    destination: str = Field(
        description="Destination path (directory or full file path) on the host machine"
    )
    ...
    try:
        ...
        if not db_url.startswith("sqlite"):
            raise http_error(
                ...
                "Backup supported only for SQLite database",
                request,
            )
...
async def download_database_backup(request: Request, backup_filename: str):
    ...
    return FileResponse(
        target_path, media_type="application/octet-stream", filename=target_path.name
    )
...
@router.post(
    "/operations/database-backups/archive/save-to-path", response_model=OperationResult
)
async def save_backups_zip_to_path(request: Request, payload: ZipSaveRequest):
    ...
    if not raw_dest:
        raise http_error(
            400, ErrorCode.BAD_REQUEST, "Destination path is required", request
        )
    # Sanitize: prevent path traversal and restrict to allowed base directory
    allowed_base = (
        Path(__file__).resolve().parents[3] / "backups" / "exports"
    ).resolve()
    dest_candidate = (allowed_base / raw_dest).resolve()
    if not str(dest_candidate).startswith(str(allowed_base)):
        raise http_error(
            400, ErrorCode.BAD_REQUEST, "Invalid destination path", request
        )
...
async def download_selected_backups_zip(request: Request, payload: ZipSelectedRequest):
    ...
    if not selected:
        raise http_error(
            404,
            ErrorCode.CONTROL_BACKUP_LIST_FAILED,
            "No valid backup files selected",
            request,
        )
    buf = io.BytesIO()
    with zipfile.ZipFile(buf, mode="w", compression=zipfile.ZIP_DEFLATED) as zf:
...
@router.post(
    "/operations/database-backups/archive/selected/save-to-path",
    response_model=OperationResult,
)
async def save_selected_backups_zip_to_path(
    request: Request, payload: ZipSelectedSaveRequest
):
    import io
    import zipfile
...
@router.post(
    "/operations/database-backups/delete-selected", response_model=OperationResult
)
...
@router.post(
    "/operations/database-backups/{backup_filename:path}/save-to-path",
    response_model=OperationResult,
)
async def save_database_backup_to_path(
    request: Request, backup_filename: str, payload: BackupCopyRequest
):
    ...
    if not str(dest_candidate).startswith(str(allowed_base)):
        raise http_error(
            400, ErrorCode.BAD_REQUEST, "Invalid destination path", request, context={}
        )
    if dest_candidate.exists() and dest_candidate.is_dir():
