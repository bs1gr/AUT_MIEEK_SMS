...
router = APIRouter(
    prefix="/admin/rbac", tags=["RBAC"], responses={404: {"description": "Not found"}}
)
...
async def bulk_assign_role(
    ...
    role = (
        ...
        .filter(models.Role.name == payload.role_name.strip().lower())
        .first()
    )
    if not role:
    ...
    for uid in payload.user_ids:
        ...
        exists = (
            ...
            .filter(
                models.UserRole.user_id == user.id, models.UserRole.role_id == role.id
            )
            .first()
        )
        if not exists:
...
async def bulk_grant_permission(
    ...
    for rname in payload.role_names:
        role = (
            db.query(models.Role)
            .filter(models.Role.name == rname.strip().lower())
            .first()
        )
        if not role:
            audit_logger.log_from_request(
...
async def ensure_defaults(
    ...
    try:
        ...
        for pname, pdesc in perm_defs:
            p = (
                db.query(models.Permission)
                .filter(models.Permission.name == pname)
                .first()
            )
            if not p:
...
async def get_rbac_summary(
    ...
    return RBACSummary(
        ...
        user_roles=[
            {"user_id": ur.user_id, "role_id": ur.role_id} for ur in user_roles
        ],
    )
...
async def revoke_role(
    ...
    try:
        ...
        if role.name == "admin":
            ...
            admin_user_roles = (
                db.query(models.UserRole)
                .filter(models.UserRole.role_id == admin_role.id)
                .all()
            )
