...
**Issue**: Latest installer (v1.18.0) defaults to PostgreSQL + Docker Compose, causing fresh installs to fail with "Failed to start Docker Compose stack"  
...
- **v1.18.0**: ❌ Contains the issue (from commit 2a3e64704 onward)
- **v1.17.9**: ✅ Uses fullstack image (no compose auto-routing)
- **v1.17.8 and earlier**: ✅ Uses fullstack image
...
3. **Update installer** with next release (v1.18.1 or v1.19.0)
