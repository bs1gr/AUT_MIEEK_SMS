...
if (Test-Path $preCommitPath) {
    ...
    if ($existingContent -notmatch "Auto-generated by scripts/setup_git_hooks.ps1") {
        Write-Host "⚠️  Existing pre-commit hook found. Renaming to pre-commit-legacy to preserve functionality." -ForegroundColor Yellow
...
#!/bin/sh
# Pre-commit hook
# Enforces Policy 2 (Version Format), Policy 5 (COMMIT_READY), and Deprecation Policy
...
if [ -f "scripts/validate_version_format.ps1" ]; then
    ...
    echo "⚠️ Warning: scripts/validate_version_format.ps1 not found. Skipping version check."
fi

# 2. Deprecation Policy Check
# Prevents committing deprecated scripts to root directory
DEPRECATION_CHECK="$HOOKS_DIR/pre-commit-deprecation-check"
if [ -f "$DEPRECATION_CHECK" ]; then
    ...
    echo "⚠️ Warning: pre-commit-deprecation-check not found. Skipping deprecation check."
fi

# 3. Run legacy/framework hooks if they exist
...
Write-Host "✅ Git pre-commit hook installed successfully at $preCommitPath" -ForegroundColor Green
