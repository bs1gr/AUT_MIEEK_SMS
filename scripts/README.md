...
Notes:
- If services are not running, the orchestrator will auto-start Docker for the smoke probe and stop it after.
- `COMMIT_MESSAGE.txt` is auto-generated by this workflow; treat it as transient output, not hand-written documentation.
...
### Installer Wizards (Removed in v1.12.3)

Legacy PowerShell installer and uninstaller helpers have been fully removed from the repository. The canonical workflow is:

- Build installers via `INSTALLER_BUILDER.ps1` (root level) which wraps `installer/SMS_Installer.iss`.
- Distribute the signed `SMS_Installer_{version}.exe` generated by Inno Setup.
- Use the auto-generated `Uninstall_SMS_{version}.exe` created by Inno Setup for removals.

**Why:** Inno Setup provides better Windows integration, code signing, and a single source of truth for installer assets without maintaining duplicate PowerShell entry points.

**User Impact:** Download the published installer executables from GitHub Releases; no PowerShell wizard scripts remain in `scripts/utils/installer/`.
